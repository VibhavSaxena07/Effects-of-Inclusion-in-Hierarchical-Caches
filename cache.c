	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /*===============================ENF-INC-V_S=================================*/
if(cp->sets[0].l1 != NULL)
{
  int i;
  for(i=0; i<(cp->bsize/cp->sets[0].l1->bsize);i++)
  {
    md_addr_t l1_repl_addr = CACHE_MK_BADDR(cp, repl->tag, set)+(i*(cp->sets[0].l1->bsize));
      cache_flush_addr(cp->sets[0].l1,l1_repl_addr,now); /* sending the location for flush*/
  }
}
/*===============================================================================*/
